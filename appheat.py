...
from fortyguard.exceptions import TaskFailedError, TaskTimeoutError

HISTORY_FILE = "fg_temp_history.json"
LOCATIONS_FILE = "fg_saved_locations.json"
...
# ──────────────────── Local history cache ───────────────────────
def save_reading(lat, lon, date_str, time_str, temp):
    history = []
    if os.path.exists(HISTORY_FILE):
        with open(HISTORY_FILE, "r") as f:
            history = json.load(f)
    history.append({
        "lat": lat, "lon": lon, "date": date_str, "time": time_str,
        "temp": temp, "fetched_at": datetime.now().isoformat(),
    })
...
def load_locations():
    if os.path.exists(LOCATIONS_FILE):
        with open(LOCATIONS_FILE, "r") as f:
            return json.load(f)
    return [
        {"name": "Lincoln Elementary (School)", "lat": 34.0522, "lon": -118.2437},
        {"name": "5th St Construction Site (Workers)", "lat": 40.7128, "lon": -74.0060},
        {"name": "Downtown Bus Stop (Public)", "lat": 29.7604, "lon": -95.3698},
    ]
...
tab1, tab2, tab3 = st.tabs(["📍 Quick Check", "🏘️ Community Dashboard", "📊 History"])
...
with tab2:
    st.markdown('<div class="section-title">🏘️ Community Monitoring Stations</div>', unsafe_allow_html=True)

    # ── Add location ──
    with st.expander("➕ Add a new monitoring station"):
        st.caption("💡 Tip: use the Search on the Quick Check tab to find coordinates, then paste them here.")
        nc1, nc2, nc3 = st.columns([2, 1, 1])
        with nc1:
            new_name = st.text_input("Station name", placeholder="e.g. Lincoln High School", key="add_name")
        with nc2:
            new_lat = st.number_input("Lat", value=st.session_state.sel_lat, format="%.5f", key="add_lat")
        with nc3:
            new_lon = st.number_input("Lon", value=st.session_state.sel_lon, format="%.5f", key="add_lon")
        if st.button("💾 Save Station"):
            if new_name.strip():
                st.session_state.locations.append({"name": new_name.strip(), "lat": new_lat, "lon": new_lon})
                save_locations(st.session_state.locations)
                st.success(f"Added **{new_name}** ✅")
                st.rerun()
            else:
                st.error("Enter a station name.")

    # ── Map of all stations ──
    if st.session_state.locations:
        loc_df = pd.DataFrame(st.session_state.locations)
        st.map(loc_df, latitude="lat", longitude="lon", zoom=3, use_container_width=True)

    # ── Remove station ──
    if st.session_state.locations:
        rc1, rc2 = st.columns([3, 1])
        with rc1:
            rm = st.selectbox("Remove station", ["—"] + [l["name"] for l in st.session_state.locations])
        with rc2:
            st.write("")
            if rm != "—" and st.button("🗑️ Remove"):
                st.session_state.locations = [l for l in st.session_state.locations if l["name"] != rm]
                save_locations(st.session_state.locations)
                st.rerun()

    st.divider()

    if st.button("📡 Scan All Stations Now", type="primary", use_container_width=True):
        now = datetime.now()
        d = now.strftime("%Y-%m-%d")
        t = now.strftime("%H:00")
        rows = []
        ...
        with st.status(f"Scanning {len(locs)} stations …", expanded=True) as status:
            for i, loc in enumerate(locs):
                st.write(f"📍 {loc['name']} ({i+1}/{len(locs)})")
                try:
                    temp = get_temperature(loc["lat"], loc["lon"], d, t)
                    exceed = get_exceedance_hours(loc["lat"], loc["lon"], d, t)
                    lvl, color = risk_level(temp, exceed)
                    rows.append({
                        "Station": loc["name"],
                        "Temp": fmt_temp(temp, use_f),
                        "Hours ≥ 35°C": f"{exceed:.1f}",
                        "Risk": lvl,
                        "Action": recommendation(lvl),
                        "_sort": {"High": 0, "Moderate": 1, "Low": 2}.get(lvl, 9),
                        "_color": color,
                    })
                    save_reading(loc["lat"], loc["lon"], d, t, temp)
                except (TaskFailedError, TaskTimeoutError, ValueError) as e:
                    rows.append({
                        "Station": loc["name"], "Temp": "—", "Hours ≥ 35°C": "—",
                        "Risk": "Error", "Action": str(e), "_sort": 9, "_color": "#94a3b8",
                    })
        ...
        if rows:
            ...
            if high:
                st.markdown(
                    ...
                    f'<span class="pulse-dot"></span>'
                    f'<b style="color:#991b1b">{high} station(s) at HIGH risk!</b> '
                    f'Review action plans below.</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.success("✅ All stations are within safe levels.")

            for r in rows:
                with st.container(border=True):
                    rc1, rc2, rc3, rc4 = st.columns([2, 1, 1, 3])
                    with rc1:
                        st.markdown(f"**{r['Station']}**")
                    with rc2:
                        st.markdown(f"🌡️ {r['Temp']}")
                    with rc3:
                        st.markdown(
                            f'<span class="risk-badge" style="background:{r["_color"]}">{r["Risk"]}</span>',
                            unsafe_allow_html=True,
                        )
                    with rc4:
                        st.caption(r["Action"])
    else:
        st.info("👆 Hit **Scan All Stations** to check heat risk for every saved location.")
...
with tab3:
    ...
    if history:
        ...
        hdf["fetched_at"] = pd.to_datetime(hdf["fetched_at"])
        hdf = hdf.sort_values("fetched_at", ascending=False)

        st.markdown(f"**{len(hdf)}** readings stored locally.")

        chart_data = hdf.copy().set_index("fetched_at").sort_index()
        if use_f:
            chart_data["temp_f"] = chart_data["temp"].apply(c_to_f)
            st.line_chart(chart_data[["temp_f"]], color="#7209b7")
        else:
            st.line_chart(chart_data[["temp"]], color="#7209b7")

        display_df = hdf[["date", "time", "lat", "lon", "temp", "fetched_at"]].copy()
        if use_f:
            display_df["temp"] = display_df["temp"].apply(lambda c: round(c_to_f(c), 1))
            display_df = display_df.rename(columns={"temp": "Temp (°F)"})
        else:
            display_df["temp"] = display_df["temp"].round(1)
            display_df = display_df.rename(columns={"temp": "Temp (°C)"})
        ...
        st.info("No readings yet. Use **Quick Check** or **Community Dashboard** to fetch data.")
...
st.markdown(
    ...
    'Built for <b>FortyGuard Hackathon \'26</b> 🏆 — Track 4: Community & Environment'
    '</div>',
    unsafe_allow_html=True,
)